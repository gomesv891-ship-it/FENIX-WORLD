...
interface NovoDocumentoModalProps {
  isOpen: boolean;
  documentoToEdit: DocumentoItem | null;
  categorias: DocumentoCategoria[];
  onClose: () => void;
  onSave: (
    data: Partial<DocumentoItem>,
    file?: File | Blob | string,
    onProgress?: (progress: { currentChunk: number; totalChunks: number; percent: number }) => void
  ) => Promise<{ success: boolean; error?: string }>;
  onOpenEditor: (initialData?: { titulo: string; categoriaId: string; categoriaNome: string }) => void;
}
...
export const NovoDocumentoModal: React.FC<NovoDocumentoModalProps> = ({
  ...
  useEffect(() => {
    ...
    const currentEditId = documentoToEdit?.id || null;
    ...
    if (justOpened || editTargetChanged) {
      ...
      if (documentoToEdit) {
        setTitulo(documentoToEdit.titulo || '');
        setCategoriaId(documentoToEdit.categoriaId || categorias[0]?.id || '');
        setDescricao(documentoToEdit.descricao || '');
        setNomeArquivo(documentoToEdit.nomeArquivoOriginal || '');
        setTamanhoArquivo(documentoToEdit.tamanhoArquivo || '');
        setTipoArquivo(documentoToEdit.tipoArquivo || 'application/pdf');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanTitle) {
      setErrorMessage('Informe o nome do documento.');
      return;
    }

    if (!categoriaId) {
      setErrorMessage('Selecione uma categoria para o documento.');
      return;
    }

    if (!selectedFile && !documentoToEdit?.fileStorageKey && !nomeArquivo) {
      setErrorMessage('Selecione o arquivo do documento para upload (PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX ou TXT).');
      return;
    }

    const selectedCat = categorias.find((c) => c.id === categoriaId);
    const catName = selectedCat ? selectedCat.nome : 'Documentos';
    ...
    try {
      const res = await onSave(
        {
          ...
          nomeArquivoOriginal: selectedFile ? selectedFile.name : nomeArquivo,
          tamanhoArquivo,
          tipoArquivo,
          isPdfImportado: true,
          isModelo: false,
      ...
      if (!res.success) {
        setErrorMessage(
          res.error ||
            'Não foi possível confirmar o salvamento no Supabase. Seus dados e arquivo foram mantidos intactos, tente novamente.'
        );
        setIsSaving(false);
        setUploadProgress(null);
        return;
      ...
      console.error('Erro no salvamento do documento:', err);
      setErrorMessage(
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900">
                {documentoToEdit ? 'Editar Documento' : 'Novo Documento'}
              </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 text-xs flex items-start gap-2.5">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-[11px] text-rose-700 font-medium">
                  Os dados preenchidos e o arquivo selecionado NÃO foram perdidos. Clique em "Tentar Novamente" abaixo para reenviar.
                </p>
              </div>
            </div>
          )}
          ...
          {saveSuccess && (
            <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-xs flex items-center gap-2.5">
              ...
              <div>
                <p className="font-bold text-emerald-900">Documento gravado com sucesso!</p>
                <p className="text-[11px] text-emerald-700">Arquivo e registro persistidos no Supabase.</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              disabled={isSaving}
              placeholder="Ex: Contrato de Prestação de Serviços, Termo de Garantia..."
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              {!documentoToEdit && (
                <button
                  ...
                  className="text-xs font-semibold text-[#0052cc] hover:underline flex items-center gap-1 cursor-pointer"
                >
                  <PenTool className="w-3 h-3" />
                  <span>Ou criar no editor</span>
                </button>
              )}
            </div>
                  ...
                  <div className="flex items-center gap-3 overflow-hidden min-w-0">
                    ...
                    <div className="truncate min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
                        ...
                        <span>•</span>
                        <span>{tamanhoArquivo || 'Tamanho desconhecido'}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div>
            ...
            <textarea
              ...
              disabled={isSaving}
              placeholder="Finalidade, cláusulas específicas, observações..."
              rows={2}
          ...
          <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
            ...
            >
              {isSaving ? (
                ...
                <span>{documentoToEdit ? 'Atualizar Documento' : 'Salvar Documento'}</span>
              )}
            </button>
          </div>
        </form>
