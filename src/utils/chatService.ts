...
export async function sendChatMessage(
  ...
  // 5. Gera notificação no sininho para os destinatários (com categoria "Chat")
  try {
    const isDirect = conversationId.startsWith('direct__');
    let targetRecipient = 'Todos';

    if (isDirect) {
      const parts = conversationId.replace('direct__', '').split('__');
      const otherUser = parts.find((u) => u !== senderNorm);
      if (otherUser) {
        targetRecipient = getChatDisplayName(otherUser);
      }
    }

    sendUserNotification({
      id: `notif_chat_${newMsg.id}`,
      category: 'Chat',
      title: `Nova mensagem de ${senderMeta.name}`,
      description: trimmed || (attachment?.fileName ? `Anexo: ${attachment.fileName}` : 'Nova mensagem no Chat corporativo.'),
      targetTab: 'Chat',
      recipientName: targetRecipient,
      authorName: senderMeta.name,
      metadata: {
        conversationId,
        messageId: newMsg.id,
        senderName: senderMeta.name,
    ...
    console.warn('Erro ao disparar notificação do chat no sininho:', notifErr);
  }
